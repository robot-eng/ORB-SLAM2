									...
									:nh("~"), spinner(0), it(nh)
{
	...
	nh.param<float>("fx", fx_, 517.3064);
	nh.param<float>("fy", fy_, 516.4692);
	nh.param<float>("cx", cx_, 318.6430);
	nh.param<float>("cy", cy_, 255.3139);
	nh.param<float>("resolution", resolution_, 0.02);
	nh.param<float>("depthfactor", depthfactor_, 1.0);//1000.0
 ...
 */
pcl::PointCloud< PointCloudMapper::PointT >::Ptr PointCloudMapper::generatePointCloud( cv::Mat& color, cv::Mat& depth,Eigen::Isometry3d &T, cv::Mat& predepth)
{
    ...
    for ( int m=0; m<depth.rows; m+=1 )
    {
        for ( int n=0; n<depth.cols; n+=1)
        {
			...
			float d_pre = predepth.ptr<float>(m)[n]/mDepthMapFactor;
            d = 8.0/(d+1e-4);
			d_pre = 8.0/(d_pre+1e-4);
			//cout << d << '\t' << d_pre << endl;
            if (d-d_pre>0.1 && d_pre!=0)
			   continue;
			if (d < 0.05 || d>5)
			{
				//if(d < 0.01)
...
//         }
	  ...
	  if(KFUpdate)
	  {
		  ...
		  {   
			  cv::Mat pre_depth;
              medianBlur(depthImgs[i],pre_depth,5);
			  
			  cv::Scalar tempVal = cv::mean( pre_depth );
              matMean = tempVal.val[0];
			  //double medianDepth = pre_depth->ComputeSceneMedianDepth(2);
              //double invMedianDepth = 1.0f/medianDepth;
			  if(lastMean==0)
                    lastMean = matMean;
              double a = lastMean / matMean;
              pre_depth.convertTo(pre_depth,CV_32F,a);
			  
			  depthImgs[i]=pre_depth.clone();
			  ...
			  int count = 0;
			  if(i>0){
				  Eigen::Matrix4f OldtoNew = mvGlobalPointCloudsPose[i-1].matrix().cast<float>()*mvGlobalPointCloudsPose[i].inverse().matrix().cast<float>();
			      Eigen::Vector3f trafoInv_t = OldtoNew.block<3,1>(0,3).cast<float>();//translation().cast<float>();
	              Eigen::Matrix3f trafoInv_R = OldtoNew.block<3,3>(0,0).cast<float>();//rotationMatrix().matrix().cast<float>();
			  
              for ( int m=0; m<depthImgs[i].rows; m++ )
              {
                  for ( int n=0; n<depthImgs[i].cols; n++)
                  {
					   Eigen::Vector3f pn = (trafoInv_R * Eigen::Vector3f(m*fxi + cxi,n*fyi + cyi,1.0f)) / depthImgs[i-1].ptr<float>(m)[n] * 8 + trafoInv_t;
                       
					   if(pn[2] > 0) {
			           float new_idepth = 1.0f / pn[2];

			           float u_new = pn[0]*new_idepth*mfx + mcx;
			           float v_new = pn[1]*new_idepth*mfy + mcy;
					   
					   if(!(u_new>2.1f && v_new>2.1f && u_new < depthImgs[i].rows-2.1f && v_new<depthImgs[i].cols-2.1f)){
						   
                           continue;
					   }
						     
                       pre_depth.ptr<float>((int)(u_new+0.5f))[(int)(v_new+0.5f)] = depthImgs[i-1].ptr<float>(m)[n];
					} 
					else{
							continue;
						}      
			  ...
			  tem_cloud1= generatePointCloud(colorImgs[i], depthImgs[i], mvGlobalPointCloudsPose[i],pre_depth); 
			  pre_depth = pre_depth * 0.6 + depthImgs[i] * 0.4;
			  depthImgs[i]=pre_depth.clone();
              lastMean = matMean;
			  if(tem_cloud1->empty())
			  continue;
...
void PointCloudMapper::shutdown()
{
	...
	string save_path = "/home/ascend/orbslam2_modified/resultPointCloudFile.pcd";
